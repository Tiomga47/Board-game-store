    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (loginField.Text == "admin" && passwordField.Text == "admin")
            {
                ...
                MySqlCommand command1 = new MySqlCommand("UPDATE `foradmin` SET `admin` = '1' WHERE `foradmin`.`admin` = 1 OR `foradmin`.`admin` = 2 OR `foradmin`.`admin` = NULL", db.getConnection());
            ...
            else
            {
                ...
                MySqlCommand command1 = new MySqlCommand("UPDATE `foradmin` SET `admin` = '2' WHERE `foradmin`.`admin` = 1 OR `foradmin`.`admin` = 2 OR `foradmin`.`admin` = NULL", db.getConnection());
                adapter1.SelectCommand = command1;
                adapter1.Fill(table);
