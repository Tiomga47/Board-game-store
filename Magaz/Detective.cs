...
namespace Magaz
{
    public partial class Detective : Form
    {
        public Detective()
        {
            InitializeComponent();
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM detectivedata", db.getConnection());
        ...
        private void label2_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM detectivedata WHERE number = @number", db.getConnection());
            ...
            if (table.Rows.Count > 0)
            {
                ...
                MySqlCommand command1 = new MySqlCommand("DELETE FROM detectivedata WHERE detectivedata.number = @number", db.getConnection());
                command1.Parameters.Add("@number", MySqlDbType.VarChar).Value = number;
                adapter1.SelectCommand = command1;
                adapter1.Fill(table);

                MessageBox.Show("Товар под номером " + number + " удален");
        ...
        #endregion
        private void Detective_Load(object sender, EventArgs e)
        {
